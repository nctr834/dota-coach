...
function App() {
  ...
  const [picked, setPicked] = useState(false); // logic regarding this is incomplete.. if i+1 changes from pos, picked should become false
  ...
  const handleChat = async (query) => {
    ...
    if (mySide != null) {
      ...
      enemyTeam = mySide === "radiant" ? dire : radiant;
      const myHero = pos ? myTeam[pos - 1] : null;
      pick = myHero ? myHero.displayName : "";
    }
    setChatLoading(true);
    try {
      const data = await processQuery(query, myTeam, enemyTeam, pick, pos, mySide);
  ...
  return (
    <div className="min-h-screen bg-[#0a0e17] text-gray-100">
          ...
          <div className="flex items-center gap-3">
            <div className="inline-flex rounded-lg border border-white/10 bg-white/[0.03] p-1 gap-0.5">
              <button
                ...
                className={`px-4 py-1.5 text-xs font-medium rounded-md transition-all cursor-pointer ${view === "draft"
              ...
              <button
                ...
                className={`px-4 py-1.5 text-xs font-medium rounded-md transition-all cursor-pointer ${view === "postgame"
                    ? "bg-indigo-500/15 text-indigo-300 shadow-sm"
                    : "text-gray-500 hover:text-gray-300 hover:bg-white/5"
                  }`}
              >
      ...
      <main className="max-w-7xl mx-auto px-6 py-8">
        {/* Both views stay mounted so switching tabs never wipes their state */}
        <div className={view === "draft" ? "space-y-8" : "hidden"}>
        {/* Team + Role selectors */}
        ...
        {score && (
          <section className="max-w-lg mx-auto">
            <div className="relative overflow-hidden rounded-xl border border-white/10 bg-white/[0.03] backdrop-blur p-5">
              ...
              <div
                className={`text-center mt-3 text-sm font-semibold ${score.delta > 0 ? "text-green-400" : score.delta < 0 ? "text-red-400" : "text-gray-400"}`}
              >
                {deltaLabel}{" "}
                {score.delta !== 0 && typeof score.delta === "number" && (
        ...
        <div className={view === "postgame" ? undefined : "hidden"}>
          <PostGame />
        </div>
      </main>
    </div>
